...
STREAM = os.getenv("LTM_STREAM", "events.ltm")
GROUP = os.getenv("LTM_GROUP", "ltm-workers")
CONSUMER = os.getenv("LTM_CONSUMER", os.uname().nodename)
BLOCK_MS = int(os.getenv("LTM_BLOCK_MS", "2000"))
...
async def handle_message(fields: Dict[str, Any]) -> None:
    ...
    # Placeholder: real implementation would write to LTM DB/Vector store
    print(f"[ltm_worker] type={etype} payload.keys={list(payload)[:5]}")
