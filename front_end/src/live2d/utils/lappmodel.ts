...
export class LAppModel extends CubismUserModel {
  ...
  /**
   * Sets the lip-sync value directly to the model's parameters.
   * @param value The lip-sync value (0.0 - 1.0).
   */
  ...
  public startMotion(
    ...
    if (motion == null) {
      fetch(`${this._modelHomeDir}${motionFileName}`)
        ...
        .then(arrayBuffer => {
          if (!arrayBuffer || arrayBuffer.byteLength === 0) return InvalidMotionQueueEntryHandleValue;
    ...
    // The actual startMotionPriority is now called inside the fetch callback for async safety
